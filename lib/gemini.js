...
export async function generateJSON(prompt, schema) {
  const data = await callGemini({
    ...
    generationConfig: {
      ...
      // Extended "thinking" turned one draft call from ~4s to ~22s in
      // testing with no meaningful quality gain for these structured,
      // well-specified tasks - not worth it against a hard 60s ceiling.

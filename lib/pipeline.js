...
const TRIAGE_AND_FACTS_SCHEMA = {
  ...
  properties: {
    ...
    namedEntities: {
      type: "ARRAY",
      items: { type: "STRING" },
      description: "Any real company, supplier, manufacturer, or competitor named in the note (e.g. \"BASF\") - never her own brand.",
    },
  },
  required: ["status", "reason", "facts", "namedEntities"],
};
...
async function triageAndExtractFacts(noteText) {
  const prompt = `Do three things with this raw note from a skincare founder, in one response.
...
3. List any real company, supplier, manufacturer, or competitor named in the note (e.g. a real brand or chemical supplier's name). She never names competitors or suppliers in her published writing - this list will be used to strip any of those names out of the draft, even though the underlying fact/story stays in.
...
async function draftWithAltOpening(noteText, facts, angle, insight, namedEntities, priorFailures) {
  const factsText = facts.map((f) => `- ${f.fact} (${f.certainty})`).join("\n");
...
const HIGH_SEVERITY_RULES = new Set(["unsupported-claim", "keep-line-missing", "named-supplier"]);
...
export async function runPipeline(noteText) {
  ...
  async function attempt(priorFailures) {
    const { draft: draftText, altOpening } = await draftWithAltOpening(noteText, facts, angle, insight, namedEntities, priorFailures);
    ...
    for (const name of namedEntities) {
      if (draftText.toLowerCase().includes(name.toLowerCase())) {
        failures.push({ rule: "named-supplier", detail: `Names "${name}" directly - she never names competitors or suppliers` });
      }
    }

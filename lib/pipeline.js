...
2. Extract every atomic, checkable fact from the note - nothing invented, nothing summarised away. Mark each "certain" if the note states it plainly, or "hedged" if the note itself hedges it (words like "probably", "I think", "seems like"). Do this regardless of your answer to (1) - a draft will be attempted from these facts either way, so an empty list only if the note genuinely contains zero extractable facts.
...
export async function runPipeline(noteText) {
  ...
  // Triage's judgment is now advisory, not a hard stop: always produce a
  // draft, however thin, so Meera decides whether it's worth using rather
  // than the pipeline deciding for her not to try. The anti-fabrication
  // safeguards (facts-only, verify, lint, strip-unsupported) still apply
  // exactly as before regardless of status - a PARK note with almost no
  // real content will come back short and full of [MEERA: ] gaps, not
  // padded with invented material.
  const triageNote = triageAndFacts.status !== "DRAFT"
    ? `Triage flagged this as ${triageAndFacts.status}: ${triageAndFacts.reason} Drafted anyway per instruction - judge for yourself whether it's worth using.`
    : null;
  ...
  if (triageNote) {
    needsCheck.unshift(`[triage] ${triageNote}`);
  }

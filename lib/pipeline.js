...
export async function runPipeline(noteText) {
  ...
  const { angle, sources, discarded } = await research(noteText, facts);
  log.research = { angle, sources, discarded };
  const sourcesText = angle ? `${angle.claim} ${angle.source_name} ${angle.date}` : "";

  const insight = await preDraftInsight(noteText, facts);
  log.insight = insight;

  let draftText = await draft(noteText, facts, angle, insight, null);
  let unsupported = await verify(draftText, facts, angle);
  let lintFailures = lintDraft(draftText, { factsText, sourcesText, allowProductClaim: facts.some((f) => /our (serum|product)/i.test(f.fact)) });
  let allFailures = [...lintFailures, ...unsupported.map((u) => ({ rule: "unsupported-claim", detail: u.sentence }))];
  log.attempt1 = { draftText, unsupported, lintFailures };

  if (allFailures.length > 0) {
    draftText = await draft(noteText, facts, angle, insight, allFailures);
    unsupported = await verify(draftText, facts, angle);
    lintFailures = lintDraft(draftText, { factsText, sourcesText, allowProductClaim: facts.some((f) => /our (serum|product)/i.test(f.fact)) });
    allFailures = [...lintFailures, ...unsupported.map((u) => ({ rule: "unsupported-claim", detail: u.sentence }))];
    log.attempt2 = { draftText, unsupported, lintFailures };
  }

  const altOpening = await alternativeOpening(draftText);
  log.altOpening = altOpening;
